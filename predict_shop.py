        ...
        for i in range(select_info.shape[0]):
            ...
            for single_wifi in user_wifi:
                ...
                if(locate[0].shape[0] != 0):
                    count = count + 1
                    score[i] = score[i] + 100 - float(shop_wifi[locate[0][0]][1]) - float(single_wifi[1])
        return score
    ...
    def get_max_redundancy(self, fraction):
        ...
        for i in range(train_dataset.shape[0]):
            ...
            temp_topn_time.append(self.top_N(select_shop=selected_shop,
                                             shop_id=train_dataset.iloc[i]['shop_id'],
                                             score=time_score))
            # if temp_topn_time > self.parameter['top_n_time']:
            #     self.parameter['top_n_time'] = temp_topn_time
            # else:
            #     pass
            # if temp_topn_wifi > self.parameter['top_n_wifi']:
            #     self.parameter['top_n_wifi'] = temp_topn_wifi
            # else:
            #     pass
            print(i)
        ...
        result = pd.DataFrame({'top_n_time':temp_topn_time,'top_n_wifi':temp_topn_wifi})
